...
namespace DataEngine.app
{
    public class DataClient
    {
        ...
        public List<Row> CreateTable(string tableName, Action<TableCreater> setup)
        {
            var builder = new TableCreater().Start(tableName);
            setup(builder);
            var action = new CreateTableAction(_db, builder);
        ...
        public List<Row> CloneTable(string sourceName ,string tableName)
        {
            var action = new CloneTableAction(_db,sourceName, tableName);
            var result = action.Run();
            _db.Notify("CloneTable" , tableName , result.Count);
            return result;
        }
    }
}
